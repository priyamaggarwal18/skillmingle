...
export default function GitHubActivity() {
    ...
    const handleConnect = async () => {
        const getRepoData = await getGithubRepo(repoUrl);
                ...
                <Card className="mt-16">
                    <CardHeader>
                        ...
                        <CardDescription className="text-xl text-violet-500">
                            Enter your GitHub repository URL to start
                            tracking changes.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                    ...
                    <div className="grid gap-6 mt-8 md:grid-cols-2 lg:grid-cols-4">
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-xl font-medium text-violet-500">
                                    Total Commits
                                </CardTitle>
                                    ...
                                    >
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-md font-medium leading-none">
                                                {event.type ===
                                                    'commit' && (
                                                    <GitCommit className="inline mr-2 h-4 w-4 text-violet-500" />
                                                )}
                                                {event.type ===
                                                    'pull_request' && (
                                                    <GitPullRequest className="inline mr-2 h-4 w-4 text-violet-500" />
                                                )}
                                                {event.type ===
                                                    'issue' && (
                                                    <AlertCircle className="inline mr-2 h-4 w-4 text-violet-500" />
                                                )}
                                                {event.message}
                                            </p>
                                            <p className="text-md text-muted-foreground text-violet-500">
                                                by {event.author}
                                            </p>
